...
```{r}
model_data <- data[,c("TotalSteps","SedentaryMinutes")]
model_data
```
```{r}
# use our data
set.seed(123)
IDs <- unique(model_data$Id)
N <- nrow(model_data)
nGroups <- length(IDs)
p <- ncol(model_data)
...
for (i in 1:niter) {
  ...
  beta_mu <- solve(t(X) %*% X) %*% t(X) %*% (z - u)
  beta_sigma <- solve(t(X) %*% X)
  beta <- mvrnorm(n = 1, mu = beta_mu, Sigma = beta_sigma)
  # Sample U using MVN
  # for each group, sample from MVN
  for (j in 1:nGroups) {
    # get the indices for the group
    group_indices <- which(data$Id == IDs[j])
    # get the data just for this group
    Xj <- X[group_indices, ]
    # then sample from MVN using the group data
    u[j] <- mvrnorm(n = 1, mu = , Sigma = )
  }
  # sample z using truncated normal
  z <- rtruncnorm(n, a=-Inf, b=Inf, mean = 0, sd = 1)
  # Sample tau using gamma
  tau <- rgamma(n = 1, shape = , rate = )
...
```{r}
# sanity check by plotting the samples
